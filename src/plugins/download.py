...
async def download(client, message):
    if message.media_group_id:
        ...
        msg = await message.reply("`Start Download File`")
        try:
            messages, session = await get_tg_link_content(message.text, client, client.userbot)
        ...
        if client.userbot.me.is_premium and session != 'bot' or session == 'user':
            ...
            messages = await client.userbot.get_messages(chat_id=messages.chat.id, message_ids=messages.id)
...
@Client.on_callback_query(filters.regex(r"^progress (?P<chatID>-?\d+) (?P<mID>\d+) (?P<user>\d+)"))
async def cancel_download(_, query):
    ...
    if query.from_user.id == int(data["user"]):
        ...
        await query.answer("This Is Not Your Download. So, dont touch on this...😡😡", show_alert=True)